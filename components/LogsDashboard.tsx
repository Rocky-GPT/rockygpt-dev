...
export function LogsDashboard() {
  ...
  /**
   * Student traffic only, by default.
   *
   * Every turn this app sends is stamped `dev`, so an unfiltered view is mostly
   * the operator watching themselves — and the questions worth reading get
   * buried under the ones you just asked. Starting at `client` makes the
   * default view the one this page exists for; the filter menu still offers
   * `dev` and `bot`.
   */
  const [selectedOrigins, setSelectedOrigins] = useState<string[]>(['client']);
  ...
  const fetchLogs = useCallback(async (showLoading = false, checkVersion = true) => {
    ...
    try {
      ...
      const res = await fetch(`/api/brain/admin/logs?${params.toString()}`);
      if (res.status === 304) {
  ...
  useEffect(() => {
    ...
    try {
      eventSource = new EventSource('/api/brain/admin/logs/stream');
  ...
  return (
    <>
      <PageHeader
        title="Chat Logs"
        subtitle="Live student turns, routes, and latency"
        actions={
          /* The live indicator, moved out of a page-owned header and into the
             shell's. Its meaning is unchanged: green means the change stream is
          ...
          <div className="flex items-center gap-2 self-end sm:self-auto shrink-0">
            ...
            <div ref={filterMenuRef} className="relative">
              ...
              {isFilterMenuOpen && (
                <div
                  role="menu"
                  className="absolute right-0 top-full z-50 mt-2 w-80 origin-top-right rounded-2xl border border-white/15 bg-neutral-900/95 p-3 shadow-2xl backdrop-blur-2xl"
                >
            ...
            <div ref={viewMenuRef} className="relative">
              ...
              {isViewMenuOpen && (
                <div
                  role="menu"
                  className="absolute right-0 top-full z-50 mt-2 w-64 origin-top-right rounded-2xl border border-white/15 bg-neutral-900/95 p-3 shadow-2xl backdrop-blur-2xl"
                >
