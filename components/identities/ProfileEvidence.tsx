...
export function ProfileEvidence({ data, section, onSection }: {
  ...
  return <section className="overflow-hidden rounded-2xl border border-white/10 bg-neutral-950/35" aria-label="Profile evidence">
    ...
    <div className="space-y-4 p-4 sm:p-5">
      ...
      {section === 'event' && <p className="rounded-lg border border-sky-500/20 bg-sky-500/5 p-3 text-xs leading-5 text-sky-100/80">Dates and times describe published event occurrences. Only explicitly linked occurrences are included; other events may exist. Missing time, location, or organizer details remain unknown.</p>}
      ...
      {section === 'event' && component && <div className="space-y-1 text-xs text-muted-foreground">
        <p>{component.requested_date ? 'Matching occurrence dates' : 'Published occurrence dates'}: <span className="text-foreground">{component.occurrence_dates?.length ? component.occurrence_dates.join(', ') : component.requested_date ? 'None returned' : 'Not published'}</span>{component.timezone ? ` · ${component.timezone}` : ''}</p>
        {component.requested_date && <p>Date filter: <span className="text-foreground">{component.requested_date}</span></p>}
      </div>}
      {!!component?.unexamined_event_candidates && <p className="text-xs text-amber-200">{component.unexamined_event_candidates} of {component.linked_event_candidates} linked event candidates were not examined. This view is partial.</p>}
